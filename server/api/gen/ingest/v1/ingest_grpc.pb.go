...
const (
	IngestService_Stream_FullMethodName = "/iot.ingest.v1.IngestService/Stream"
)
...
type IngestServiceClient interface {
	...
	Stream(ctx context.Context, opts ...grpc.CallOption) (grpc.ClientStreamingClient[Reading, StreamSummary], error)
}
...
func (c *ingestServiceClient) Stream(ctx context.Context, opts ...grpc.CallOption) (grpc.ClientStreamingClient[Reading, StreamSummary], error) {
	cOpts := append([]grpc.CallOption{grpc.StaticMethod()}, opts...)
	stream, err := c.cc.NewStream(ctx, &IngestService_ServiceDesc.Streams[0], IngestService_Stream_FullMethodName, cOpts...)
	if err != nil {
		return nil, err
	}
	x := &grpc.GenericClientStream[Reading, StreamSummary]{ClientStream: stream}
	return x, nil
}

// This type alias is provided for backwards compatibility with existing code that references the prior non-generic stream type by name.
type IngestService_StreamClient = grpc.ClientStreamingClient[Reading, StreamSummary]
...
type IngestServiceServer interface {
	// Stream is client-streaming: the device opens it once and pushes readings
	// continuously. The server returns a summary when the client half-closes.
	Stream(grpc.ClientStreamingServer[Reading, StreamSummary]) error
...
func (UnimplementedIngestServiceServer) Stream(grpc.ClientStreamingServer[Reading, StreamSummary]) error {
...
func _IngestService_Stream_Handler(srv interface{}, stream grpc.ServerStream) error {
	return srv.(IngestServiceServer).Stream(&grpc.GenericServerStream[Reading, StreamSummary]{ServerStream: stream})
}

// This type alias is provided for backwards compatibility with existing code that references the prior non-generic stream type by name.
type IngestService_StreamServer = grpc.ClientStreamingServer[Reading, StreamSummary]
...
var IngestService_ServiceDesc = grpc.ServiceDesc{
	ServiceName: "iot.ingest.v1.IngestService",
